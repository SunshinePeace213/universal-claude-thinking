...
describe('Module Loading Integration', () => {
  ...
  beforeEach(() => {
    ...
    moduleLoader = {
      ...
      async loadModule(modulePath) {
        ...
        // Validate path
        if (!modulePath.startsWith('./') || !modulePath.includes('.claude/') || modulePath.includes('..')) {
      ...
      getTotalLoadTime() {
        return Array.from(this.loadTimes.values()).reduce((sum, time) => sum + time, 0);
      }
    };
  });
  ...
  describe('Module Loading with Classification', () => {
    test('should load correct modules for category A (simple)', async () => {
      ...
      expect(modules[0].path).toBe('./.claude/thinking-modules/response-formats.md');
    });
    ...
    test('should load correct modules for category B (complex)', async () => {
      ...
      expect(modules.map(m => m.path)).toContain('./.claude/thinking-modules/SAGE.md');
      expect(modules.map(m => m.path)).toContain('./.claude/thinking-modules/SEIQF.md');
      expect(modules.map(m => m.path)).toContain('./.claude/cognitive-tools/analysis.md');
    });
    ...
    test('should load correct modules for category C (search)', async () => {
      ...
      expect(modules.map(m => m.path)).toContain('./.claude/thinking-modules/SIA.md');
      expect(modules.map(m => m.path)).toContain('./.claude/cognitive-tools/search.md');
    });
    ...
    test('should load correct modules for category D (code)', async () => {
      ...
      expect(modules).toHaveLength(2);
      expect(modules.map(m => m.path)).toContain('./.claude/thinking-modules/SEIQF.md');
      expect(modules.map(m => m.path)).toContain('./.claude/cognitive-tools/code-analysis.md');
    });
    ...
    test('should load correct modules for category E (meta)', async () => {
      ...
      expect(modules).toHaveLength(2);
      expect(modules.map(m => m.path)).toContain('./.claude/thinking-modules/SAGE.md');
      expect(modules.map(m => m.path)).toContain('./.claude/cognitive-tools/meta-reasoning.md');
    });
  });
  ...
  describe('Fallback Mechanisms', () => {
    ...
    test('should handle multiple module failures', async () => {
      ...
      const result = await loadModulesWithFallback(['SAGE', 'SEIQF', 'SIA'], failingLoader);
    ...
    test('should use inline fallback when fallback module also fails', async () => {
      ...
      const result = await loadModulesWithFallback(['SAGE'], failingLoader, true);
      
      expect(result.inlineFallback).toBe(true);
      expect(result.fallbackContent).toContain('basic reasoning without specialized modules');
    });
  });
  ...
  describe('Performance Requirements', () => {
    test('should load modules within 50ms each', async () => {
      ...
      for (const module of modules) {
        const startTime = Date.now();
        await moduleLoader.loadModule(`./.claude/thinking-modules/${module}.md`);
  ...
  describe('Path Validation', () => {
    test('should reject paths outside .claude directory', async () => {
      await expect(moduleLoader.loadModule('../outside.md')).rejects.toThrow('Invalid module path');
      await expect(moduleLoader.loadModule('/absolute/path.md')).rejects.toThrow('Invalid module path');
      await expect(moduleLoader.loadModule('./other-dir/module.md')).rejects.toThrow('Invalid module path');
    });
    
    test('should reject paths with directory traversal', async () => {
      await expect(moduleLoader.loadModule('./.claude/../../../etc/passwd')).rejects.toThrow('Invalid module path');
      await expect(moduleLoader.loadModule('./.claude/modules/../../../secret.md')).rejects.toThrow('Invalid module path');
    });
...
async function loadModulesForCategory(category, loader) {
  ...
  for (const moduleName of moduleNames) {
    ...
    try {
      ...
      modules.push({ path, error: error.message });
    }
  }
...
async function loadModulesWithFallback(moduleNames, loader, testInlineFallback = false) {
  const failed = [];
  const fallbacks = [];
  ...
  for (const moduleName of moduleNames) {
    try {
      ...
      if (testInlineFallback) {
        try {
          await loader.loadModule(`./.claude/thinking-modules/${fallbackName}.md`);
