...
describe('CLAUDE.md Orchestrator', () => {
  ...
  beforeAll(async () => {
    ...
    orchestratorContent = await fs.readFile(filePath, 'utf8');
  });
  ...
  describe('Module Loading Templates', () => {
    test('should contain @import statements for each category', () => {
      // Category A
      expect(orchestratorContent).toContain('@import "./thinking-modules/response-formats.md"');
      
      // Category B
      expect(orchestratorContent).toContain('@import "./thinking-modules/SAGE.md"');
      expect(orchestratorContent).toContain('@import "./thinking-modules/SEIQF.md"');
      
      // Category C
      expect(orchestratorContent).toContain('@import "./thinking-modules/SIA.md"');
      
      // Category D
      expect(orchestratorContent).toContain('@import "./cognitive-tools/code-analysis.md"');
      
      // Category E
      expect(orchestratorContent).toContain('@import "./cognitive-tools/meta-reasoning.md"');
    });
...
describe('Request Classifier', () => {
  ...
  const classifyRequest = (request) => {
    ...
    if (/what is|how do|explain|define/.test(text)) return { category: 'A', confidence: 0.9 };
    if (/analyze|compare|evaluate|design/.test(text)) return { category: 'B', confidence: 0.85 };
    if (/find|search|locate|where|list/.test(text)) return { category: 'C', confidence: 0.9 };
    if (/write|implement|fix|debug|refactor/.test(text)) return { category: 'D', confidence: 0.9 };
    if (/how do you|your process|why did you/.test(text)) return { category: 'E', confidence: 0.85 };
    ...
    return {
      ...
      generateHeader() {
        const modules = this.activeModules
          .map(m => `${m.name} (${m.tokens < 1000 ? m.tokens : `${(m.tokens/1000).toFixed(1)}K`})`)
          .join(', ');
