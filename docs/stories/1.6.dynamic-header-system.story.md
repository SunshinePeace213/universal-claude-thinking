...
## Tasks / Subtasks
- [ ] Task 1: Create core monitoring infrastructure (AC: 1, 2, 3, 4, 5)
  - [ ] Implement StatusCollector class in src/core/monitoring/
  - [ ] Create EventBus for real-time status propagation
  - [ ] Define standardized status message schemas
- [ ] Task 2: Build header formatting system (AC: 6)
  - [ ] Implement HeaderFormatter with template support
  - [ ] Create display adapters for different output modes
  - [ ] Integrate with existing Claude-v3.md protocol headers
  - [ ] Extend CLAUDE-v3.md headers with cognitive state information
...
1. **Real-time Updates**: Current headers are static per response. Add WebSocket/EventBus integration for live status changes during processing.
